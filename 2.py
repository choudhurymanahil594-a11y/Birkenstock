...
# 在这里设置你想要采集的目标二级分类名称
TARGET_LEVEL2_CATEGORY = "Featured"
...
async def main():
    ...
    # 步骤 1: 从JSON文件中读取分类信息
    try:
        with open('birkenstock_categories_full.json', 'r', encoding='utf-8') as f:
            categories_data = json.load(f)
    except FileNotFoundError:
        print("错误: 未找到 birkenstock_categories_full.json 文件。")
        return
    except json.JSONDecodeError:
        print("错误: 解析 birkenstock_categories_full.json 文件失败。")
        return

    # 步骤 2: 查找目标二级分类下的所有三级分类信息（包括名称和URL）
    third_level_categories_to_scrape = []
    for level1_cat in categories_data:
        for level2_cat in level1_cat.get('sub_categories', []):
            if level2_cat.get('level2_category') == TARGET_LEVEL2_CATEGORY:
    ...
    if not third_level_categories_to_scrape:
        print(f"未找到分类为 '{TARGET_LEVEL2_CATEGORY}' 的任何三级分类。")
        return
    ...
    print(f"找到 {len(final_third_level_categories)} 个唯一的三级分类进行采集，分类为 '{TARGET_LEVEL2_CATEGORY}'。")
