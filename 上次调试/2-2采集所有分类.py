...
async def main():
    ...
    try:
        ...
        print("错误: 解析 navigation_data.json 文件失败。")
        return
    ...
    async with async_playwright() as p:
        ...
        for category_data in final_third_level_categories:
            ...
            category_data['product_urls'] = scraped_urls
            total_product_urls_count += len(category_data['product_urls'])
            
            if not scraped_urls:
                urls_without_products.append(url)
                print(f"在三级分类 '{category_data['level3_category']}' ({url}) 页面上未找到任何产品链接。")
            else:
    ...
    if urls_without_products:
        with open('未找到任何产品链接.json', 'w', encoding='utf-8') as f:
            json.dump(urls_without_products, f, ensure_ascii=False, indent=4)
        print(f"已将 {len(urls_without_products)} 个未找到产品链接的URL保存到 未找到任何产品链接.json 文件。")
    else:
        print("所有分类页面都找到了产品链接，未生成 未找到任何产品链接.json 文件。")
